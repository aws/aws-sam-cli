...
def _translate_to_cfn(tf_json: dict, output_directory_path: str, terraform_application_dir: str) -> dict:
    ...
    lambda_resources_to_code_map: Dict[str, List[Tuple[str, Dict]]] = {}
    ...
    while module_queue:
        ...
        for resource in resources:
            ...
            resource_address = f"data.{resource_type}.{resource_name}" if resource_mode == "data" else f"{resource_type}.{resource_name}"
            ...
            if config_resource_address not in curr_tf_module.resources:
                raise PrepareHookException(
                    f"There is no configuration resource for resource address {resource_full_address} and configuration "
            ...
            if (
                ...
                if resource.get("values", {}).get("triggers", {}).get("resource_name"):
                    sam_metadata_resources.append(SamMetadataResource(curr_module_address, resource, config_resource))
                else:
                    sam_metadata_resources = [SamMetadataResource(curr_module_address, resource, config_resource)] + sam_metadata_resources
                continue
            ...
            if resource_type == "aws_s3_object":
                ...
                config_code_artifact = code_artifact if code_artifact else _resolve_resource_attribute(config_resource, "source")
            ...
            if resource_type == TF_AWS_LAMBDA_LAYER_VERSION:
                ...
                references = translated_resource.get("Properties", {}).get("Content")
                if not references or not isinstance(references, str):
                    references = _resolve_resource_attribute(config_resource, "filename")
                if references:
                    hash_value = f"layer_{_calculate_configuration_attribute_value_hash(references)}"
                    layers_list = lambda_resources_to_code_map.get(hash_value, [])
                    layers_list.append((translated_resource, logical_id))
                    lambda_resources_to_code_map[hash_value] = layers_list
            ...
            if resource_type == TF_AWS_LAMBDA_FUNCTION:
                ...
                if translated_resource.get("Properties", {}).get("PackageType", "Zip") == "Zip":
                    references = translated_resource.get("Properties", {}).get("Code")
                    func_type = "zip"
                    tf_code_property = "filename"
                else:
                    references = translated_resource.get("Properties", {}).get("Code", {}).get("ImageUri")
                    func_type = "image"
                    tf_code_property = "image_uri"

                if not references or not isinstance(references, str):
                    references = _resolve_resource_attribute(config_resource, tf_code_property)
                if references:
                    hash_value = f"{func_type}_{_calculate_configuration_attribute_value_hash(references)}"
    ...
    if sam_metadata_resources:
        ...
        _enrich_resources_and_generate_makefile(
            sam_metadata_resources, cfn_dict.get("Resources", {}), output_directory_path, terraform_application_dir, lambda_resources_to_code_map
        )
    else:
...
def _enrich_resources_and_generate_makefile(
    ...
    for sam_metadata_resource in sam_metadata_resources:
        ...
        lambda_resources = _get_relevant_cfn_resource(sam_metadata_resource, cfn_resources, lambda_resources_to_code_map)
...
def _get_relevant_cfn_resource(
    sam_metadata_resource: SamMetadataResource, cfn_resources: Dict[str, Dict], lambda_resources_to_code_map: Dict
) -> List[Tuple[Dict, str]]:
    """
    ...
    if not resource_name:
        artifact_property_name = "built_output_path" if resource_type in ["ZIP_LAMBDA_FUNCTION", "LAMBDA_LAYER"] else "built_image_uri"
        references = sam_metadata_resource.resource.get("values", {}).get("triggers", {}).get(artifact_property_name)
        if not references:
            references = _resolve_resource_attribute(sam_metadata_resource.config_resource, artifact_property_name)
        hash_value = f"{resources_types[resource_type]}_{_calculate_configuration_attribute_value_hash(references)}"
...
def _map_s3_sources_to_functions(s3_hash_to_source: Dict[str, Tuple[str, List[Union[ConstantValue, ResolvedReference]]]], cfn_resources: Dict[str, Any], lambda_resources_to_code_map: Dict[str, List[Tuple[str, Dict]]]) -> None:
    """
