...
# todo: remove this once Skip Infra Sync is GA
ENABLE_SKIP_INFRA_SYNC = True
...
class InfraSyncExecutor:
    ...
    def execute_infra_sync(self, first_sync: bool = False) -> InfraSyncResult:
        ...
        if ENABLE_SKIP_INFRA_SYNC and (first_sync and (days_since_last_infra_sync <= AUTO_INFRA_SYNC_DAYS)):
            ...
            try:
                if self._auto_skip_infra_sync(
                    ...
                    if len(self.code_sync_resources) < SYNC_FLOW_THRESHOLD:
                        ...
                        LOG.info(
                            "The number of resources that needs an update exceeds %s, an infra sync will be "
                            "executed for an CloudFormation deployment to improve performance",
                            SYNC_FLOW_THRESHOLD,
                        )
                        pass
