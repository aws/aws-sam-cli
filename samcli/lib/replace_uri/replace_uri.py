...
class ReplaceLocalCodeUri:
    ...
    def _replace_local_codeuri(self):
        ...
        all_resources = self.template.get("Resources", {})
        global_settings = self.template.get("Globals", {})
        ...
        for resource_type, properties in global_settings.items():
            ...
            if resource_type == "Function":
                if all(
                    ...
                    ReplaceLocalCodeUri._update_to_s3_uri("CodeUri", properties)
        ...
        for _, resource in all_resources.items():
            ...
            if resource_type == "AWS::Serverless::Function" and resource_dict.get("PackageType", ZIP) == ZIP:
                ...
                ReplaceLocalCodeUri._update_to_s3_uri("CodeUri", resource_dict)

            if resource_type == "AWS::Serverless::LayerVersion":

                ReplaceLocalCodeUri._update_to_s3_uri("ContentUri", resource_dict)

            if resource_type == "AWS::Serverless::Api":
                if "DefinitionUri" in resource_dict:
                    ReplaceLocalCodeUri._update_to_s3_uri("DefinitionUri", resource_dict)
