...
def check_nested_stack(client, stack_name):
    ...
    nested_stacks = [logical_id for logical_id, value in template.get("Resources", {}).items() if
                     value.get("Type") == "AWS::CloudFormation::Stack"]

    click.confirm(
        'The stack {} is a nested stack with the following resources: {}. Are you want to destroy them?'.format(
            stack_name, nested_stacks), default=True,
        abort=True)
...
def do_cli(ctx, stack_name, retain_resources=None, role_arn=None, client_request_token=None, wait=None, wait_time=None,
    ...
    try:
        ...
        if "TerminationProtection" in e.response["Error"]["Message"]:
            msg = 'The stack {stack_name} has TerminationProtection turned on. Disable it on the aws console at ' + \
            ...
            click.secho(
                ...
                'Please update the resources required to delete the stack and the required user policies.', fg="red")
        else:
