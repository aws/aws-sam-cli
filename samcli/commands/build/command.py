...
def do_cli(template, source_root, build_dir, clean, use_container):
    """
    ...
    with BuildContext(template,
                      ...
                      use_container=use_container) as ctx:

        builder = ApplicationBuilder(ctx.function_provider,
                                     ctx.build_dir,
                                     ctx.source_root)
        artifacts = builder.build()
        modified_template = builder.update_template(ctx.template_dict, artifacts)
