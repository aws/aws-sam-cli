...
import tomlkit
from tomlkit.api import _TOMLDocument as TOMLDocument  # type: ignore[attr-defined]
...
def _sync_state_to_toml_document(sync_state: SyncState) -> TOMLDocument:
    sync_state_toml_table = tomlkit.table()  # type: ignore[attr-defined]
    sync_state_toml_table[DEPENDENCY_LAYER] = sync_state.dependency_layer

    toml_document = tomlkit.document()  # type: ignore[attr-defined]
    toml_document.add((tomlkit.comment("This file is auto generated by SAM CLI sync command")))  # type: ignore[attr-defined]
    toml_document.add(SYNC_STATE, cast(Item, sync_state_toml_table))

    return toml_document


def _toml_document_to_sync_state(toml_document: Dict) -> Optional[SyncState]:  # type: ignore[type-arg]
...
class SyncContext:
    ...
    def __enter__(self) -> "SyncContext":
        ...
        if self._previous_state and self._previous_state.dependency_layer != self._current_state.dependency_layer:
            self._cleanup_build_folders()  # type: ignore[no-untyped-call]
    ...
    def _write(self) -> None:
        with open(self._file_path, "w+") as file:
            file.write(tomlkit.dumps(_sync_state_to_toml_document(self._current_state)))  # type: ignore[attr-defined]

    def _read(self) -> None:
        try:
            with open(self._file_path) as file:
                toml_document = cast(Dict, tomlkit.loads(file.read()))  # type: ignore[type-arg, attr-defined]
    ...
    def _cleanup_build_folders(self):  # type: ignore[no-untyped-def]
        """
