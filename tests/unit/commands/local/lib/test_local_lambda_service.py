...
class TestLocalLambdaService(TestCase):
    ...
    @patch("samcli.commands.local.lib.local_lambda_service.LocalLambdaHttpService")
    def test_start(self, local_lambda_http_service_mock):
        lambda_runner_mock = Mock()
        stderr_mock = Mock()
        lambda_invoke_context_mock = Mock()

        lambda_context_mock = Mock()
        local_lambda_http_service_mock.return_value = lambda_context_mock
        ...
        local_lambda_http_service_mock.assert_called_once_with(
