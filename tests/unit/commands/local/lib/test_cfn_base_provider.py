...
class TestDisableAuthorizerFlagInExtractSwaggerRoute(TestCase):
    @parameterized.expand([
        ("when enabled will not extract authorizers", True),
        ("when disabled extracts authorizers", False)
    ])
    ...
    def test_disabled_authorizer_flag(self, _, disable_authorizer, mock_reader: Mock, mock_parser: Mock, mock_debug_log: Mock, mock_get_authorizers: Mock):
        mock_collector = Mock()
        CfnBaseApiProvider.extract_swagger_route(stack_path="", logical_id="", body={}, uri="", binary_media=None, collector=mock_collector, cwd="", event_type="HttpApi", disable_authorizer=disable_authorizer)
