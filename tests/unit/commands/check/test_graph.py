...
from samcli.commands.check.resources.graph import CheckGraph, _check_input


class TestGraph(TestCase):
    @patch("samcli.commands.check.resources.graph.CheckGraph")
    def test_generate(self, patch_graph):
        ...
        graph._make_connections = Mock()
        graph._find_entry_points = Mock()

        graph.generate()

        graph._make_connections.assert_called_once()
        graph._find_entry_points.assert_called_once()
    ...
    def test_handle_lambda_permissions(self):
        ...
        self_mock._lambda_functions = {function_name_mock: function_object_mock}
        self_mock._api_gateways = {source_name_mock: source_object_mock}

        permission_mock = Mock()
        permission_mock.resource_object = permission_object

        self_mock._lambda_permissions.values.return_value = [permission_mock]

        CheckGraph._handle_lambda_permissions(self_mock)
        ...
        self_mock._lambda_functions = {}

        with self.assertRaises(Exception):
            CheckGraph._handle_lambda_permissions(self_mock)

    @patch("samcli.commands.check.resources.graph.DynamoDB")
    def test_handle_event_source_mappings(self, patch_dynamo):
        ...
        self_mock._event_source_mappings.values.return_value = [event_mock]
        self_mock._lambda_functions = {function_name_mock: function_object_mock}

        self_mock._dynamo_db_tables = {}

        CheckGraph._handle_event_source_mappings(self_mock)
    ...
    def test_find_entry_points(self):
        ...
        self_mock._lambda_functions.values.return_value = [function_object_mock]
        self_mock._api_gateways.values.return_value = []
        self_mock._dynamo_db_tables.values.return_value = []

        CheckGraph._find_entry_points(self_mock)

        self.assertEqual(self_mock.entry_points[0], function_object_mock)

        self_mock._api_gateways.values.return_value = [api_object_mock]
        CheckGraph._find_entry_points(self_mock)

        self.assertEqual(self_mock.entry_points[-1], api_object_mock)

        self_mock._dynamo_db_tables.values.return_value = [dynamo_object_mock]
        CheckGraph._find_entry_points(self_mock)

        self.assertEqual(self_mock.entry_points[-1], dynamo_object_mock)

    def test_make_connections(self):
        self_mock = Mock()

        self_mock._handle_lambda_permissions = Mock()
        self_mock._handle_event_source_mappings = Mock()
        self_mock._handle_iam_roles = Mock()

        CheckGraph._make_connections(self_mock)

        self_mock._handle_lambda_permissions.assert_called_once()
        self_mock._handle_event_source_mappings.assert_called_once()
    ...
    def test_handle_iam_roles(self):
        ...
        self_mock._lambda_functions = {lambda_function_name: lambda_function_mock}
        ...
        CheckGraph._handle_iam_roles(self_mock)
    ...
    def test_get_properties(self):
        ...
        result = CheckGraph._get_properties(self_mock, lambda_function_mock)
        ...
        with self.assertRaises(Exception):
            CheckGraph._get_properties(self_mock, lambda_function_mock)
    ...
    def test_make_connection_from_policy(self):
        ...
        CheckGraph._make_connection_from_policy(self_mock, child_resource_type, lambda_function_name)
    ...
    @patch("samcli.commands.check.resources.graph._check_input")
    @patch("samcli.commands.check.resources.graph.click")
    def test_ask_dynamo_connection_question(self, patch_click, patch_check):
        ...
        self_mock = Mock()
        self_mock._dynamo_db_tables = {dynamo_table_name: selected_resource_mock}
        ...
        result = CheckGraph._ask_dynamo_connection_question(self_mock, lambda_function_name)
