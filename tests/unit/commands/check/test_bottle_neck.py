...
from samcli.commands.check.bottle_necks import BottleNecks, lambda_bottle_neck_quesitons, ask, ask_bottle_neck_questions
...
class TestBottleNeck(TestCase):
    ...
    @patch("samcli.commands.check.bottle_necks.ask_bottle_neck_questions")
    @patch("samcli.commands.check.bottle_necks.ask")
    @patch("samcli.commands.check.bottle_necks.click")
    def test_ask_entry_point_question(self, click_patch, patch_ask, patch_questions):
        graph_mock = Mock()
        ...
        patch_questions.assert_called_once_with(entry_point_mock)
        graph_mock.add_resource_to_analyze.assert_called_once_with(entry_point_mock)

    @patch("samcli.commands.check.bottle_necks.lambda_bottle_neck_quesitons")
    def test_ask_bottle_neck_questions(self, patch_questions):
        my_resource = Mock()
        my_resource.get_resource_type.return_value = "AWS::Lambda::Function"

        ask_bottle_neck_questions(my_resource)

        patch_questions.assert_called_with(my_resource)
    ...
    def test_lambda_bottle_neck_quesitons(self, patch_ask):
        ...
        lambda_bottle_neck_quesitons(lambda_function_mock)
