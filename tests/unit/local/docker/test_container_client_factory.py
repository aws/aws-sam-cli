...
class TestValidateAdminContainerPreference(TestCase):
    ...
    @patch("samcli.cli.context.Context")
    @patch("samcli.local.docker.container_client_factory.LOG")
    def test_validate_admin_container_preference_valid_inputs(
        self, input_value, expected_output, log_value, mock_log, mock_context
    ):
        """Test validation with valid container runtime inputs"""

        # Create a real Context object
        real_ctx = Context()
        mock_context.get_current_context.return_value = real_ctx
    ...
    @patch("samcli.cli.context.Context")
    @patch("samcli.local.docker.container_client_factory.LOG")
    def test_validate_admin_container_preference_invalid_inputs(self, input_value, mock_log, mock_context):
...
class TestAdminContainerPreference(TestCase):
    ...
    @patch("samcli.local.docker.container_client_factory.LOG")
    @patch("samcli.local.docker.container_client_factory.get_platform_handler")
    def test_admin_preference_valid_inputs(self, raw_config, expected_result, mock_get_handler, mock_log, mock_context):
    ...
    @patch("samcli.local.docker.container_client_factory.LOG")
    @patch("samcli.local.docker.container_client_factory.get_platform_handler")
    def test_admin_preference_invalid_inputs(self, raw_config, mock_get_handler, mock_log, mock_context):
