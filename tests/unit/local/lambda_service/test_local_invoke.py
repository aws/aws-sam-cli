...
class TestLocalLambdaService(TestCase):
    ...
    def test_initalize_creates_default_values(self):
        ...
        service = LocalInvoke(lambda_runner=lambda_runner_mock, port=3001, host='127.0.0.1')
    ...
    def test_initalize_with_values(self):
        lambda_runner_mock = Mock()
        stderr_mock = Mock()
        local_service = LocalInvoke(lambda_runner_mock, port=5000, host='129.0.0.0', stderr=stderr_mock)
    ...
    @patch('samcli.local.lambda_service.local_invoke.LocalInvoke._construct_error_handling')
    @patch('samcli.local.lambda_service.local_invoke.Flask')
    def test_create_service_endpoints(self, flask_mock, error_handling_mock):
        ...
        lambda_runner_mock = Mock()
        service = LocalInvoke(lambda_runner=lambda_runner_mock, port=3000, host='localhost')
    ...
    @patch('samcli.local.lambda_service.local_invoke.LocalInvoke._service_response')
    @patch('samcli.local.lambda_service.local_invoke.LambdaOutputParser')
    @patch('samcli.local.lambda_service.local_invoke.request')
    ...
    def test_request_handler_returns_process_stdout_when_making_response(self, request_mock, lambda_output_parser_mock,
        ...
        lambda_runner_mock = Mock()
        stderr_mock = Mock()
        service = LocalInvoke(lambda_runner=lambda_runner_mock, port=3000, host='localhost', stderr=stderr_mock)
