...
class TestStartAPICDKTemplateRestAPI(StartApiIntegBaseClass):
    ...
    def test_invoke_api(self):
        response = requests.get(self.url + "/v1", timeout=300)
...
class TestStartAPICDKRestAPIWithAssets(StartApiIntegBaseClass):
    template_path = "/testdata/start_api/cdk/api_with_assets.yaml"
    ...
    @pytest.mark.timeout(timeout=600, method="thread")
    def test_invoke_api(self):
        response = requests.get(self.url + "/get-item", timeout=300)

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {"hello": "world"})


class TestStartAPICDKHttpAPI(StartApiIntegBaseClass):
    template_path = "/testdata/start_api/cdk/template-http-api.yaml"
    ...
    @pytest.mark.timeout(timeout=600, method="thread")
    def test_invoke_api(self):
        response = requests.get(self.url + "/get-info", timeout=300)
