...
class BuildIntegBase(TestCase):
    ...
    def tearDown(self):
        ...
        """
        # Clean up Docker images and layers created more than 3 minutes ago
        ...
        if self.custom_build_dir and os.path.exists(self.custom_build_dir):
            try:
                shutil.rmtree(self.custom_build_dir)
                LOG.debug(f"Cleaned up custom_build_dir: {self.custom_build_dir}")
            except Exception as e:
                LOG.warning(f"Failed to clean up custom_build_dir {self.custom_build_dir}: {e}")

        # Clean up working directory (contains default_build_dir) if it exists
        if self.working_dir and os.path.exists(self.working_dir):
            try:
                shutil.rmtree(self.working_dir)
                LOG.debug(f"Cleaned up working_dir: {self.working_dir}")
            except Exception as e:
                LOG.warning(f"Failed to clean up working_dir {self.working_dir}: {e}")

        # Clean up scratch directory (parent of all test artifacts) if it exists
        if self.scratch_dir and os.path.exists(self.scratch_dir):
            try:
                shutil.rmtree(self.scratch_dir)
                LOG.debug(f"Cleaned up scratch_dir: {self.scratch_dir}")
            except Exception as e:
    ...
    def _cleanup_old_docker_images(self):
        ...
        try:
            ...
            three_minutes_ago = current_time.timestamp() - 180  # 3 minutes in seconds
            ...
            for image in all_images:
                try:
                    ...
                    if created_timestamp < three_minutes_ago:
...
@pytest.mark.dotnet
class BuildIntegDotnetBase(BuildIntegBase):
    FUNCTION_LOGICAL_ID = "Function"
    EXPECTED_FILES_PROJECT_MANIFEST = {
    ...
    def validate_build_command(self, overrides, mode, mount_mode=None, use_container=False, input=None) -> None:
        ...
        if input:
            command_result = run_command_with_input(cmdlist, input.encode(), cwd=self.working_dir, env=newenv)
        else:
            command_result = run_command(cmdlist, cwd=self.working_dir, env=newenv)

        self.assertEqual(command_result.process.returncode, 0)

    def validate_invoke_command(self, overrides, runtime):
        expected = "{'message': 'Hello World'}"
    ...
    def validate_build_artifacts(self, expected_manifest):
        self._verify_built_artifact(
            self.default_build_dir,
            self.FUNCTION_LOGICAL_ID,
            expected_manifest,
        ...
        self._verify_built_properties()

    def _verify_built_properties(self):
        self._verify_resource_property(
            str(self.built_template),
            "OtherRelativePathResource",
            "BodyS3Location",
