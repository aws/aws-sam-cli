...
class TestPublishExistingApp(PublishAppIntegBase):
    ...
    def test_update_application(self):
        template_path = str(self.test_data_path.joinpath("template_update_app.yaml"))
        ...
        app_metadata_text = self.test_data_path.joinpath("metadata_update_app.json").read_text()
        app_metadata = json.loads(app_metadata_text)
        self.assert_metadata_details(app_metadata, process_stdout.decode('utf-8'))

    def test_create_application_version(self):
        template_path = str(self.test_data_path.joinpath("template_create_app_version.yaml"))
        command_list = self.get_command_list(template_path=template_path, region=self.region_name)
        ...
        app_metadata_text = self.test_data_path.joinpath("metadata_create_app_version.json").read_text()
        app_metadata = json.loads(app_metadata_text)
        self.assert_metadata_details(app_metadata, process_stdout.decode('utf-8'))
...
class TestPublishNewApp(PublishAppIntegBase):
    ...
    def test_create_application(self):
        template_path = str(self.test_data_path.joinpath("template_create_app.yaml"))
        command_list = self.get_command_list(template_path=template_path, region=self.region_name)
    ...
    def test_publish_not_packaged_template(self):
        template_path = str(self.test_data_path.joinpath("template_not_packaged.yaml"))
        command_list = self.get_command_list(template_path=template_path, region=self.region_name)
