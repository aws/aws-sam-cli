...
resource "null_resource" "build_lambda_function" {
    ...
    provisioner "local-exec" {
        command = "powershell.exe -File .\\PyBuild.ps1 ${local.lambda_src_path} ${local.building_path} ${local.lambda_code_filename}"
    }
}
...
resource "null_resource" "build_layer_version" {
    ...
    provisioner "local-exec" {
        command = "powershell.exe -File .\\PyBuild.ps1 ${local.layer_src_path} ${local.building_path} ${local.layer_code_filename}"
    }
}
