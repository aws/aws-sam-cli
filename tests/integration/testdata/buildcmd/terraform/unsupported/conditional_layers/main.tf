...
resource "aws_lambda_function" "function1" {
    ...
    layers = var.stage == "gamma" ? [aws_lambda_layer_version.layer1[0].arn] : [aws_lambda_layer_version.layer1[1].arn]
    depends_on = [
...
resource "null_resource" "sam_metadata_aws_lambda_layer_version_layer1_0" {
    triggers = {
...
resource "null_resource" "sam_metadata_aws_lambda_layer_version_layer1_1" {
    triggers = {
        resource_name = "aws_lambda_layer_version.layer1[1]"
        resource_type = "LAMBDA_LAYER"
