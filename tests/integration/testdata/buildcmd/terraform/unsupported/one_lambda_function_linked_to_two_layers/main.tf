...
resource "null_resource" "build_layer2_version" {
    ...
    provisioner "local-exec" {
        command = "./py_build.sh \"${local.layer2_src_path}\" \"${local.building_path}\" \"${local.layer2_artifact_filename}\" Layer"
    }
}

resource "null_resource" "sam_metadata_aws_lambda_layer_version_layer2" {
    triggers = {
        resource_name = "aws_lambda_layer_version.layer2[0]"
        resource_type = "LAMBDA_LAYER"

        original_source_code = local.layer2_src_path
        built_output_path = "${local.building_path}/${local.layer2_artifact_filename}"
    }
    depends_on = [
...
resource "aws_lambda_layer_version" "layer2" {
    count = 1
    filename = "${local.building_path}/${local.layer2_artifact_filename}"
    layer_name = "${var.namespace}_lambda_layer2"
    compatible_runtimes = ["python3.8", "python3.9"]
    depends_on = [
        null_resource.build_layer2_version
    ]
