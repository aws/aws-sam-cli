...
class TestSyncWatchCode(TestSyncWatchBase):
    ...
    def test_sync_watch_code(self):
        ...
        read_until_string(
            self.watch_process, "Finished syncing Lambda Function HelloWorldFunction", timeout=30
        )
        lambda_functions = self.stack_resources.get(AWS_LAMBDA_FUNCTION)
        for lambda_function in lambda_functions:
            lambda_response = json.loads(self._get_lambda_response(lambda_function))
        ...
        read_until_string(self.watch_process, "Finished syncing RestApi HelloWorldApi", timeout=20)
        ...
        read_until_string(self.watch_process, "Finished syncing StepFunctions HelloStepFunction", timeout=20)
        state_machine = self.stack_resources.get(AWS_STEPFUNCTIONS_STATEMACHINE)[0]
...
class TestSyncCodeWatchNestedStacks(TestSyncWatchBase):
    ...
    def test_sync_watch_code_nested_stack(self):
        ...
        if self.dependency_layer:
            ...
            read_until_string(
                self.watch_process,
                "Finished syncing Function Layer Reference Sync " "LocalNestedChildStack/HelloWorldFunction",
                timeout=45,
            )
            layer_contents = self.get_dependency_layer_contents_from_arn(self.stack_resources, "python", 2)
            self.assertIn("requests", layer_contents)
        ...
        read_until_string(
            self.watch_process,
            "Finished syncing Lambda Function LocalNestedChildStack/HelloWorldFunction",
            timeout=30,
        )
        lambda_functions = self.stack_resources.get(AWS_LAMBDA_FUNCTION)
        for lambda_function in lambda_functions:
        ...
        read_until_string(
            self.watch_process,
            "Finished syncing RestApi LocalNestedChildStack/HelloWorldApi",
            timeout=20,
        )
        time.sleep(API_SLEEP)
        rest_api = self.stack_resources.get(AWS_APIGATEWAY_RESTAPI)[0]
        ...
        read_until_string(
            self.watch_process,
            "Finished syncing StepFunctions LocalNestedChildStack/HelloStepFunction",
            timeout=20,
        )
        state_machine = self.stack_resources.get(AWS_STEPFUNCTIONS_STATEMACHINE)[0]
        time.sleep(SFN_SLEEP)
...
class TestSyncWatchCodeUseContainer(TestSyncWatchUseContainer):
    ...
    def test_sync_watch_code(self):
        ...
        read_until_string(
            self.watch_process, "Finished syncing Lambda Function HelloWorldFunction", timeout=45
        )
...
class TestSyncWatchCodeOnly(TestSyncWatchBase):
    ...
    def test_sync_watch_code(self):
        ...
        self.watch_process = start_persistent_process(sync_command_list, cwd=self.test_data_path)
        read_until_string(self.watch_process, "Sync watch started", timeout=30)
...
class TestSyncWatchAutoSkipInfra(SyncIntegBase):
    ...
    def test_sync_watch_auto_skip_infra(self):
        ...
        read_until_string(
            self.watch_process, "Finished syncing Lambda Function HelloWorldFunction", timeout=100
        )
        ...
        read_until_string(
            self.watch_process,
            "Finished syncing RestApi HelloWorldApi",
            timeout=100,
        )
        time.sleep(API_SLEEP)
        kill_process(self.watch_process)
        ...
        read_until_string(
            self.watch_process,
            "Finished syncing StepFunctions HelloStepFunction",
            timeout=100,
        )
        time.sleep(SFN_SLEEP)
