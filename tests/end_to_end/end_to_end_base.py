...
class EndToEndBase(InitIntegBase, StackOutputsIntegBase, DeleteIntegBase, SyncIntegBase):
    ...
    def _get_remote_invoke_command(self, stack_name, resource_id, event, output_format):
        return RemoteInvokeIntegBase.get_command_list(
            stack_name=stack_name, resource_id=resource_id, event=event, output_format=output_format
        )
