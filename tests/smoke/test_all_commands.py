...
class TestAllCommands(TestCase):
  ...
  def test_package(self, template_file_name):
    self.run_and_verify_no_crash("package", [
      "--template-file", os.path.join(TEMPLATE_FOLDER, template_file_name), 
      "--s3-bucket", "sdfafds-random-bucket"
    ])

  @parameterized.expand(TEMPLATE_FILE_NAMES)
  def test_deploy(self, template_file_name):
    self.run_and_verify_no_crash("deploy", [
      "--template-file", os.path.join(TEMPLATE_FOLDER, template_file_name), 
      "--stack-name", "dsfafs-random-stack"
    ])
