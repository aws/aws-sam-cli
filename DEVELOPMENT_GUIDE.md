...
- For dependencies used in SAM CLI code, update `base.txt`, `reproducible-linux.txt` in `requirements` folder and `THIRD-PART-LICENSESD` in `installer/assets` folder. To update `base.txt` file, simply follow the current convention and input the dependency name plus version, together with any necessary comment. For more information on the operators to be used for restricting compatible versions, read on [python's enhancement proposals](https://peps.python.org/pep-0440/#compatible-release). To update the `reproducible-linux.txt`, run the following script to replace the file:
  ```
